...
static void print_test_namespace(struct ieee80211_radiotap_iterator *iter) {
    ...
    case 52:
        printf("\t00:00:00-00|%d: %.2x/%.2x/%.2x/%.2x\n", iter->this_arg_index,
               *iter->this_arg, *(iter->this_arg + 1), *(iter->this_arg + 2),
               *(iter->this_arg + 3));
        break;
    default:
...
void got_packet(u_char *args, const struct pcap_pkthdr *header,
    ...
    while (!(err = ieee80211_radiotap_iterator_next(&iter))) {
        if (iter.this_arg_index == IEEE80211_RADIOTAP_VENDOR_NAMESPACE) {
            printf("\tvendor NS (%.2x-%.2x-%.2x:%d, %d bytes)\n",
                   iter.this_arg[0], iter.this_arg[1], iter.this_arg[2],
                   iter.this_arg[3], iter.this_arg_size - 6);
    ...
    struct ieee80211_hdr *hdr =
        (struct ieee80211_hdr *)(packet + iter._max_length);
...
int main(int argc, char *argv[]) {
    ...
    char *fname;    /* The pcap file to open */
    bpf_u_int32 netp;
    char errbuf[PCAP_ERRBUF_SIZE]; /* Error string */
    struct bpf_program fp;         /* The compiled filter */
    char filter_exp[] =
        "type mgt subtype assoc-req"; /* The filter expression */
    ...
    */
    ...
    if (pcap_loop(handle, num_packets, got_packet, handle) < 0) {
        fprintf(stderr, "\npcap_loop() failed: %s\n", pcap_geterr(handle));
        return 0;
    }
